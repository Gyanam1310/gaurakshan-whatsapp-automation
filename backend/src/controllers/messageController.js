...
const { getSocket } = require("../config/whatsapp");
const {
  initializeWhatsApp,
  isWhatsAppConnected,
  getWhatsAppState,
...
const GROUP_ID_PATTERN = /^\d{5,}-\d+@g\.us$/;
...
async function sendMessage(req, res) {
  ...
  } else if (!GROUP_ID_PATTERN.test(groupId)) {
    validationErrors.push("groupId must match format like 1234567890-123456789@g.us");
  }
...
async function testWhatsApp(req, res) {
  ...
  if (!GROUP_ID_PATTERN.test(payload.groupId)) {
    return res.status(400).json({
      success: false,
      ...incomingBody,
      id: payload.id,
      error: "groupId must match format like 1234567890-123456789@g.us",
    });
  }
